...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # 先将所有factors作为当前factors
        curFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminationVar in eliminationOrder:
            # join factor，并获取此时的factor列表，更新curFactors
            curFactors, joinedFactor = joinFactorsByVariable(curFactors, eliminationVar)
            if len(joinedFactor.unconditionedVariables()) != 1:
                # 非条件变量仍然多于一，继续消除变量
                curFactors.append(eliminate(joinedFactor, eliminationVar))
            else:
                # 非条件变量等于一，消元结束，获取下一个var进行消元
                continue
        return normalize(joinFactors(curFactors))
